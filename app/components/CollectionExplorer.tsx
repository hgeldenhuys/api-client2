...
export function CollectionExplorer() {
  ...
  const [exportCollectionId, setExportCollectionId] = React.useState<
    string | undefined
  >();
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (overIdParts.length >= 3) {
      ...
      const targetItem = findItemInCollection(
        ...
        {isFolderItem(item) && isExpanded && (
          <>
            {item.item.map((subItem) =>
              renderItem(subItem, collectionId, depth + 1),
            )}
          </>
        )}
      </DraggableTreeItem>
          ...
          {collections.size === 0 ? (
            ...
            Array.from(collections.entries())
              .filter(
                ([_, { collection }]) =>
                  !searchQuery ||
                  matchesSearch(collection.info.name, searchQuery) ||
              ...
              .map(([id, { collection }]) => {
                ...
                const expandedFolders =
                  collections.get(id)?.metadata.expandedFolders || [];
