...
export function RequestBuilder() {
  ...
  const { isLoading, executeRequest, abortRequest, setActiveRequest } =
    useRequestStore();
  ...
  const [bodyMode, setBodyMode] = React.useState<
  ...
  const handleSendRequest = async () => {
    ...
    const currentRequest =
      activeCollectionId && activeRequestId
        ? findRequestById(activeCollectionId, activeRequestId)
        : null;
    ...
    const headerParams = grouped.header.filter(
      (p) => p.enabled && p.key.trim(),
    );

    // Build body from universal parameters
    let requestBody: any = undefined;
    const formDataParams = grouped["form-data"].filter(
      (p) => p.enabled && p.key.trim(),
    );
    const urlencodedParams = grouped.urlencoded.filter(
      (p) => p.enabled && p.key.trim(),
    );
    ...
    if (formDataParams.length > 0) {
      ...
      const formBody = urlencodedParams
        .map(
          (p) =>
            `${encodeURIComponent(replaceVariables(p.key))}=${encodeURIComponent(replaceVariables(p.value))}`,
        )
        .join("&");
      requestBody = {
        mode: "urlencoded",
  ...
  const exportOptions = [
    {
      label: "Copy as HTTP",
      onClick: () => copyAsCode("http"),
      icon: <Copy className="h-4 w-4" />,
    },
    {
      label: "Copy as cURL",
      onClick: () => copyAsCode("curl"),
      icon: <Copy className="h-4 w-4" />,
    },
    {
    ...
    {
      label: "Copy as C#",
      onClick: () => copyAsCode("csharp"),
      icon: <Code className="h-4 w-4" />,
    },
    {
      label: "Copy as Swift",
      onClick: () => copyAsCode("swift"),
      icon: <Code className="h-4 w-4" />,
    },
    {
      label: "Copy as PowerShell",
      onClick: () => copyAsCode("powershell"),
      icon: <Code className="h-4 w-4" />,
    },
  ];
  ...
  const collection = activeCollectionId
    ? collections.get(activeCollectionId)
    : null;
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-4 border-b">
        <div className="flex gap-2">
          ...
          {isLoading ? (
            ...
            <SplitButton onClick={handleSendRequest} options={exportOptions}>
      ...
      <div className="flex-1 overflow-hidden">
        <Tabs defaultValue="params" className="h-full">
            ...
            <div className="space-y-4">
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <Select
                    value={bodyMode}
                    onValueChange={(v) => setBodyMode(v as any)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
          ...
          <TabsContent
            value="auth"
            className="h-[calc(100%-40px)] p-4 overflow-y-auto"
          >
            ...
            <div className="h-full flex flex-col">
              <div className="p-3 border-b flex items-center justify-between">
                <p className="text-sm text-muted-foreground">
                  Pre-request scripts run before the request is sent. Use the pm
                  object to set variables or modify the request.
                </p>
          ...
          <TabsContent value="tests" className="h-[calc(100%-40px)] p-0">
            <div className="h-full flex flex-col">
              <div className="p-3 border-b flex items-center justify-between">
                <p className="text-sm text-muted-foreground">
                  Test scripts run after the response is received. Write tests
