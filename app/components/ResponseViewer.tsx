...
export function ResponseViewer() {
  ...
  const copyToClipboard = () => {
    const textToCopy = responseView === 'pretty' ? formatJson(currentResponse?.body) : getBodyAsString();
  ...
  const downloadResponse = () => {
    ...
    const content = responseView === 'pretty' ? formatJson(currentResponse.body) : getBodyAsString();
  ...
  const getContentType = (): string => {
    ...
    return currentResponse.headers['content-type'] || currentResponse.headers['Content-Type'] || '';
  };
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className={`font-medium ${getStatusColor(currentResponse?.status || 0)}`}>
            ...
            {testScriptResult.tests.filter(t => !t.passed).length > 0 && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-red-600">
                  {testScriptResult.tests.filter(t => !t.passed).length} failed
                </span>
              </div>
            )}
          </div>
        ...
        <Tabs defaultValue="body" className="h-full">
          <TabsList className="w-full justify-start rounded-none border-b h-auto p-0">
            ...
            <TabsTrigger 
              value="tests" 
              className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary"
              disabled={!testScriptResult || testScriptResult.tests.length === 0}
            >
              Test Results
            </TabsTrigger>
            <TabsTrigger 
              value="console" 
              className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary"
              disabled={(!preRequestScriptResult || preRequestScriptResult.consoleOutput.length === 0) && (!testScriptResult || testScriptResult.consoleOutput.length === 0)}
            >
              Console
            </TabsTrigger>
          </TabsList>
            ...
            <div className="h-full flex flex-col">
              <div className="border-b p-2">
                <div className="flex gap-1">
                  ...
                  {canPreview() && (
                    <Button
                      variant={responseView === 'preview' ? 'secondary' : 'ghost'}
                      size="sm"
                    ...
                    {currentResponse.isCorsError ? (
                      <Card className="border-orange-200 bg-orange-50 dark:bg-orange-950/20">
                        <div className="p-4 space-y-3">
                          ...
                          {proxyStore.isEnabled && proxyStore.proxyStatus !== 'connected' && (
                            <div className="bg-orange-100 dark:bg-orange-900/30 rounded-md p-3">
                              <p className="text-sm text-orange-800 dark:text-orange-200">
                                Proxy is enabled but not connected. Make sure your proxy server is running at{' '}
                      ...
                      <>
                        {getContentType().includes('html') ? (
                          ...
                          <div className="text-sm text-muted-foreground p-4">
                            Preview is only available for HTML and Markdown responses
                          </div>
                        )}
                      </>
                    )}
          ...
          <TabsContent value="headers" className="h-[calc(100%-40px)] overflow-auto p-4">
          ...
          <TabsContent value="tests" className="h-[calc(100%-40px)] overflow-auto p-4">
            {testScriptResult && (
              <div className="space-y-2">
                {testScriptResult.tests.map((test, index) => (
                  <Card key={index} className="p-3">
                    ...
                    {test.error && (
                      <p className="text-sm text-red-600 mt-1 ml-6">{test.error}</p>
                    )}
                  </Card>
                ))}
              </div>
          ...
          <TabsContent value="console" className="h-[calc(100%-40px)] overflow-auto p-4">
            <div className="space-y-4 font-mono text-sm">
              {preRequestScriptResult && preRequestScriptResult.consoleOutput.length > 0 && (
                <div className="space-y-2">
              ...
              {testScriptResult && testScriptResult.consoleOutput.length > 0 && (
                <div className="space-y-2">
