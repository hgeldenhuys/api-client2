...
import { getLocationChangeImpact, validateLocationChange } from '~/utils/parameterLocationSync';
...
export const UniversalParametersEditor = React.memo(function UniversalParametersEditor({ 
  ...
  const [selectedLocation, setSelectedLocation] = React.useState<ParameterLocation | 'all'>('all');
  const [showDisabled, setShowDisabled] = React.useState(true);

  const groupedParameters = groupParametersByLocation(parameters);
  
  const filteredParameters = selectedLocation === 'all' 
    ? parameters 
  ...
  const handleParameterChange = React.useCallback((id: string, updates: Partial<UniversalParameter>) => {
  ...
  const handleAddParameter = React.useCallback(() => {
    const newParam = createUniversalParameter('', '', selectedLocation === 'all' ? 'query' : selectedLocation);
    onChange([...parameters, newParam]);
  }, [parameters, onChange, selectedLocation]);

  const handleRemoveParameter = React.useCallback((id: string) => {
  ...
  const getLocationCounts = () => {
    ...
    Object.keys(PARAMETER_LOCATIONS).forEach(location => {
      counts[location] = groupedParameters[location as ParameterLocation].length;
    });
    return counts;
  };
    ...
    <div className={cn("h-full flex flex-col", className)}>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 p-4 border-b">
        <div className="flex items-center gap-3 flex-wrap">
          <Select value={selectedLocation} onValueChange={(value) => setSelectedLocation(value as ParameterLocation | 'all')}>
            ...
            <SelectContent>
              ...
              {Object.values(PARAMETER_LOCATIONS).map(locationInfo => (
                <SelectItem key={locationInfo.location} value={locationInfo.location}>
                  <span className="flex items-center gap-2">
                    <span>{locationInfo.icon}</span>
                    {locationInfo.label} ({locationCounts[locationInfo.location]})
                  </span>
                </SelectItem>
              ))}
            </SelectContent>
          ...
          >
            {showDisabled ? <Eye className="h-4 w-4 mr-1" /> : <EyeOff className="h-4 w-4 mr-1" />}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Button variant="outline" size="sm" className="whitespace-nowrap" onClick={() => handleBulkToggle(true)}>
            Enable All
          </Button>
          
          <Button variant="outline" size="sm" className="whitespace-nowrap" onClick={() => handleBulkToggle(false)}>
            Disable All
          </Button>

          <Button onClick={handleAddParameter} size="sm" className="whitespace-nowrap">
      ...
      <div className="flex-1 overflow-hidden">
        <Tabs defaultValue="table" className="h-full">
          <TabsList className="w-full justify-start rounded-none border-b h-auto p-0">
            <TabsTrigger value="table" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary">
              Parameters Table
            </TabsTrigger>
          ...
          <TabsContent value="table" className="h-[calc(100%-40px)] overflow-y-auto p-0">
              ...
              <div className="divide-y">
                {displayParameters.length === 0 ? (
                  ...
                  Object.entries(groupedParameters).map(([location, params]) => 
                    params.filter(p => !showDisabled ? p.enabled : true).length > 0 && (
                      <div key={location}>
                        {/* Location group header */}
                        <div className="bg-muted/30 px-3 py-2 border-b">
                          <div className="flex items-center gap-2 text-sm font-medium text-muted-foreground">
                            <span>{PARAMETER_LOCATIONS[location as ParameterLocation].icon}</span>
                            <span>{PARAMETER_LOCATIONS[location as ParameterLocation].label}</span>
                            <Badge variant="secondary" className="text-xs">
                              {params.filter(p => !showDisabled ? p.enabled : true).length}
                            </Badge>
                          </div>
                        </div>
                        ...
                        {params
                          ...
                          .map((param) => (
                            <ParameterRow
                              key={param.id}
                              parameter={param}
                              onChange={(updates) => handleParameterChange(param.id, updates)}
          ...
          <TabsContent value="preview" className="h-[calc(100%-40px)] overflow-y-auto p-4">
...
const ParameterRow = React.memo(function ParameterRow({ parameter, onChange, onRemove }: ParameterRowProps) {
  ...
  const handleLocationChange = React.useCallback((value: string) => {
    ...
    if (!validation.valid) {
      ...
      console.warn(`Cannot move parameter to ${newLocation}: ${validation.reason}`);
      return;
    }
    
    // Get impact information (useful for debugging or future UI enhancements)
    const impact = getLocationChangeImpact([parameter], parameter.id, newLocation);
  ...
  const handleEnabledChange = React.useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        <div>
          ...
          <div className="bg-muted/50 p-3 rounded-md font-mono text-sm space-y-1">
            {grouped['form-data']
              ...
              .map(p => (
                <div key={p.id}>
                  {p.key}: {p.type === 'file' ? `[FILE: ${p.src || 'No file selected'}]` : replaceVariables(p.value)}
                </div>
              ))}
          </div>
        </div>
