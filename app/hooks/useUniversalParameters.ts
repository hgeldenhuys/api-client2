...
export function useUniversalParameters({
  ...
  const createParametersHash = React.useCallback(
    (params: UniversalParameter[]) => {
      return JSON.stringify(
        params.map(
          (p) => `${p.id}:${p.key}:${p.value}:${p.location}:${p.enabled}`,
        ),
      );
    },
    [],
  ...
  React.useEffect(() => {
    ...
    if (parsed.isValid) {
      const currentQueryParams = parameters.filter(
        (p) => p.location === "query",
      );
  ...
  const loadParameters = React.useCallback(
    (requestData: any) => {
      ...
      if (
        requestData.universalParameters &&
        Array.isArray(requestData.universalParameters)
      ) {
        ...
        const urlString =
          typeof requestData.request?.url === "string"
        ...
        if (
          requestData.request?.body?.mode === "x-www-form-urlencoded" &&
          requestData.request.body.raw
        ) {
