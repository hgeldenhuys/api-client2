...
export interface VariableContextValue {
  ...
  addVariable: (key: string, value: string, scope: 'global' | 'environment') => void;
  updateVariable: (key: string, value: string, scope: 'global' | 'environment') => void;
...
export function useVariableContext(): VariableContextValue {
  ...
  const addVariable = useCallback((key: string, value: string, scope: 'global' | 'environment') => {
  ...
  }, [activeEnvironmentId, environments, setGlobalVariable, addEnvironmentVariable]);

  const updateVariable = useCallback((key: string, value: string, scope: 'global' | 'environment') => {
    if (scope === 'global') {
  ...
  }, [activeEnvironmentId, globalVariables, updateGlobalVariable, updateEnvironmentVariable]);

  const deleteVariable = useCallback((key: string, scope: 'global' | 'environment') => {
    if (scope === 'global') {
      const globalVarIndex = globalVariables.findIndex(v => v.key === key);
      if (globalVarIndex >= 0) {
  ...
  }, [activeEnvironmentId, globalVariables, deleteGlobalVariable, deleteEnvironmentVariable]);

  const hasVariable = useCallback((key: string) => {
