...
export class OpenAPIExporter {
  ...
  private static createOpenAPIDocument(
    ...
    Object.entries(pathGroups).forEach(([path, requests]) => {
      ...
      requests.forEach((request) => {
        const operation = this.convertRequestToOpenAPIOperation(
          request,
          options,
          warnings,
        );
  ...
  private static createSwaggerDocument(
    ...
    warnings.push(
      "Swagger 2.0 export has limited feature support compared to OpenAPI 3.0+",
    );
    ...
    Object.entries(pathGroups).forEach(([path, requests]) => {
      ...
      requests.forEach((request) => {
        const operation = this.convertRequestToSwaggerOperation(
          request,
          options,
          warnings,
        );
   ...
   */
  private static groupRequestsByPath(
  ...
  private static convertRequestToOpenAPIOperation(
    ...
    try {
      ...
      if (
        request.request.url &&
        typeof request.request.url === "object" &&
      ...
      if (
        ["POST", "PUT", "PATCH"].includes(request.request.method) &&
        request.request.body
      ) {
        const contentType =
          this.getContentType(request.request.header) || "application/json";
        ...
        operation.requestBody = {
          ...
          content: {
            [contentType]: {
              schema: this.generateSchemaFromBody(
                request.request.body,
                options.generateSchemas,
              ),
            },
          },
        };
   ...
   */
  private static generateSchemaFromBody(
    body: any,
   ...
   */
  private static convertPostmanAuthToOpenAPI(
    auth: RequestAuth,
