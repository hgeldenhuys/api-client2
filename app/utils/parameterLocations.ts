...
export const PARAMETER_LOCATIONS: Record<
  ParameterLocation,
  ParameterLocationInfo
> = {
  query: {
...
export function validateParameter(param: UniversalParameter): {
  ...
  if (rules.maxLength && param.key.length > rules.maxLength) {
    errors.push(
      `Key too long for ${locationInfo.label} (max ${rules.maxLength})`,
    );
  }
  ...
  if (rules.maxLength && param.value.length > rules.maxLength) {
    errors.push(
      `Value too long for ${locationInfo.label} (max ${rules.maxLength})`,
    );
  }
