...
function createPMObject(context: ScriptContext) {
      ...
      have: {
        status: (expectedStatus: number) => {
          if (context.response!.code !== expectedStatus) {
            throw new Error(`Expected status to be ${expectedStatus}, but got ${context.response!.code}`);
          }
        }
      }
    }
  ...
  const pm = {
    ...
    sendRequest: async (options: any) => {
      ...
      throw new Error('pm.sendRequest is not yet implemented in script execution');
    }
  };
...
class ChaiExpect {
  ...
  equal(expected: any) {
    ...
    if (this.negated ? isEqual : !isEqual) {
      throw new Error(`Expected ${this.actual} ${this.negated ? 'not ' : ''}to equal ${expected}`);
    }
  }
  ...
  eql(expected: any) {
    ...
    if (this.negated ? isEqual : !isEqual) {
      throw new Error(`Expected ${JSON.stringify(this.actual)} ${this.negated ? 'not ' : ''}to deep equal ${JSON.stringify(expected)}`);
    }
  }
  ...
  a(type: string) {
    ...
    if (this.negated ? isType : !isType) {
      throw new Error(`Expected ${this.actual} ${this.negated ? 'not ' : ''}to be a ${type}`);
    }
  }
  ...
  get ok() {
    if (this.negated ? this.actual : !this.actual) {
      throw new Error(`Expected ${this.actual} ${this.negated ? 'not ' : ''}to be truthy`);
    }
    return this;
  }
  ...
  get true() {
    if (this.negated ? this.actual === true : this.actual !== true) {
      throw new Error(`Expected ${this.actual} ${this.negated ? 'not ' : ''}to be true`);
    }
    return this;
  }
  ...
  get false() {
    if (this.negated ? this.actual === false : this.actual !== false) {
      throw new Error(`Expected ${this.actual} ${this.negated ? 'not ' : ''}to be false`);
    }
    return this;
  }
  ...
  get null() {
    if (this.negated ? this.actual === null : this.actual !== null) {
      throw new Error(`Expected ${this.actual} ${this.negated ? 'not ' : ''}to be null`);
    }
    return this;
  }
  ...
  get undefined() {
    if (this.negated ? this.actual === undefined : this.actual !== undefined) {
      throw new Error(`Expected ${this.actual} ${this.negated ? 'not ' : ''}to be undefined`);
    }
    return this;
  }
  ...
  above(n: number) {
    if (this.negated ? this.actual > n : this.actual <= n) {
      throw new Error(`Expected ${this.actual} ${this.negated ? 'not ' : ''}to be above ${n}`);
    }
  }
  
  below(n: number) {
    if (this.negated ? this.actual < n : this.actual >= n) {
      throw new Error(`Expected ${this.actual} ${this.negated ? 'not ' : ''}to be below ${n}`);
    }
  }
  ...
  include(value: any) {
    ...
    if (this.negated ? includes : !includes) {
      throw new Error(`Expected ${JSON.stringify(this.actual)} ${this.negated ? 'not ' : ''}to include ${JSON.stringify(value)}`);
    }
  }
  ...
  match(pattern: RegExp) {
    ...
    if (this.negated ? matches : !matches) {
      throw new Error(`Expected ${this.actual} ${this.negated ? 'not ' : ''}to match ${pattern}`);
    }
  }
  ...
  property(name: string, value?: any) {
    ...
    if (value !== undefined) {
      ...
      if (this.negated ? propertyValue === value : propertyValue !== value) {
        throw new Error(`Expected property ${name} ${this.negated ? 'not ' : ''}to equal ${value}`);
      }
    }
  }
  ...
  length(n: number) {
    ...
    if (this.negated ? actualLength === n : actualLength !== n) {
      throw new Error(`Expected length ${this.negated ? 'not ' : ''}to be ${n}, but got ${actualLength}`);
    }
  }
  ...
  status(code: number) {
    ...
    if (this.negated ? this.actual.code === code : this.actual.code !== code) {
      throw new Error(`Expected status ${this.negated ? 'not ' : ''}to be ${code}, but got ${this.actual.code}`);
    }
  }
  ...
  oneOf(values: any[]) {
    ...
    if (this.negated ? isOneOf : !isOneOf) {
      throw new Error(`Expected ${this.actual} ${this.negated ? 'not ' : ''}to be one of ${JSON.stringify(values)}`);
    }
  }
}
    ...
    try {
      ...
      try {
        ...
        const result: WorkerResult = {
          ...
          requestUpdates: Object.keys(requestUpdates).length > 0 ? requestUpdates : undefined
        };
